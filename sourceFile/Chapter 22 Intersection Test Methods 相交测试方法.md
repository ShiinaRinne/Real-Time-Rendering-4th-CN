...
<div style="text-align: center;">
    <img style="border-radius: 0.3125em; box-shadow: 0 2px 4px 0 rgba(34,36,38,.12),0 2px 10px 0 rgba(34,36,38,.08);" src="images/Chapter-22/202310081205104.png">
</div>
<div style="color:orange;display: block;color: #999;padding: 2px; text-align: left;">
    图22.18：一个中心点为 \mathbf{c} ，正半对角线为 \mathbf{h} 的AABB，在平面 \pi 上进行测试。其思想是计算从box中心到平面的带符号距离 s ，并将其与box的“范围” e 进行比较。图中向量 \mathbf{g}^i 是这个二维box的不同对角线，而在这个例子中 \mathbf{h} 等于 \mathbf{g}^1 。还需要注意的是，当这个带符号距离 s 为负，且其绝对值大于 e 时，则代表这个box位于位于平面内部（ s + e < 0 ）。
</div>
<br><br>
...
<div style="text-align: center;">
    <img style="border-radius: 0.3125em; box-shadow: 0 2px 4px 0 rgba(34,36,38,.12),0 2px 10px 0 rgba(34,36,38,.08);" src="images/Chapter-22/202310081612824.png">
</div>
<div style="color:orange;display: block;color: #999;padding: 2px; text-align: left;">
    图22.19：三角形和它们所在的平面。在两幅图中，交集部分使用红色进行标记。左：沿直线 L 的交叉部分发生重叠，这两个三角形也相交。右：两个三角形没有交集，三角形在直线L上的对应部分也不重叠。
</div>
<br><br>
...
<div style="text-align: center;">
    <img style="border-radius: 0.3125em; box-shadow: 0 2px 4px 0 rgba(34,36,38,.12),0 2px 10px 0 rgba(34,36,38,.08);" src="images/Chapter-22/202310081623220.png">
</div>
<div style="color:orange;display: block;color: #999;padding: 2px; text-align: left;">
    图22.20：螺旋矢量 \mathbf{b}-\mathbf{a} 在 \mathbf{d}-\mathbf{c} 方向上的图示。
</div>
<br><br>
...
<div style="text-align: center;">
    <img style="border-radius: 0.3125em; box-shadow: 0 2px 4px 0 rgba(34,36,38,.12),0 2px 10px 0 rgba(34,36,38,.08);" src="images/Chapter-22/202310081717702.png">
</div>
<div style="color:orange;display: block;color: #999;padding: 2px; text-align: left;">
    图22.21：用于三角形与box重叠测试的符号表示。在左边，展示了box和三角形的初始位置。在右边，box和三角形进行了平移，使得box的中心与原点重合。
</div>
<br><br>
...
<div style="text-align: center;">
    <img style="border-radius: 0.3125em; box-shadow: 0 2px 4px 0 rgba(34,36,38,.12),0 2px 10px 0 rgba(34,36,38,.08);" src="images/Chapter-22/202310081949419.png">
</div>
<div style="color:orange;display: block;color: #999;padding: 2px; text-align: left;">
    图22.23：为了确定两个OBB是否重叠，可以使用分离轴试验。这里我们以二维形式进行展示。图中的四个分离轴与两个OBB的面正交，每个box都有两个这样的轴。然后，OBB被投影到这些轴上。如果两个投影在所有轴上都发生重叠，那么则代表这两个OBB重叠；否则，则代表这两个OBB不重叠。因此，找到一个 能够 分离投影的轴，就足以知道这两个OBB不重叠了。而在这个例子中，左下方的轴是唯一能够分离投影的轴。 [436]
</div>
<br><br>
...
<div style="text-align: center;">
    <img style="border-radius: 0.3125em; box-shadow: 0 2px 4px 0 rgba(34,36,38,.12),0 2px 10px 0 rgba(34,36,38,.08);" src="images/Chapter-22/202310082012691.png">
</div>
<div style="color:orange;display: block;color: #999;padding: 2px; text-align: left;">
    图22.24：左边是一个无限延伸的金字塔，这个金字塔被相互平行的近裁剪平面和远裁剪平面裁剪成一个视锥体。右侧还展示了各个平面的名称，相机的位置在金字塔的顶端。
</div>
<br><br>
...
<div style="text-align: center;">
    <img style="border-radius: 0.3125em; box-shadow: 0 2px 4px 0 rgba(34,36,38,.12),0 2px 10px 0 rgba(34,36,38,.08);" src="images/Chapter-22/202310082054550.png">
</div>
<div style="color:orange;display: block;color: #999;padding: 2px; text-align: left;">
    图22.25：左上角展示了一个视锥体（蓝色）和一个一般的包围体（绿色），其中选择了一个相对于物体的点 \mathbf{p} 。通过追踪物体在视锥体的外部（右上）和视锥体内部（左下）移动时的点 \mathbf{p} ，尽可能地靠近视锥体，可以将视锥体与BV的相交测试，重新定义为测试点 \mathbf{p} 与外部体积和内部体积之间的测试，如右下角所示。如果点 \mathbf{p} 位于橙色体积外部，则说明该BV也位于视锥体外部；如果点 \mathbf{p} 位于橙色区域内，则说明该BV与视锥体相交；如果点 \mathbf{p} 位于紫色区域内，则说明该BV完全位于视锥体内部。
</div>
<br><br>
...
<div style="text-align: center;">
    <img style="border-radius: 0.3125em; box-shadow: 0 2px 4px 0 rgba(34,36,38,.12),0 2px 10px 0 rgba(34,36,38,.08);" src="images/Chapter-22/202310091023296.png">
</div>
<div style="color:orange;display: block;color: #999;padding: 2px; text-align: left;">
    图22.26：左边是一个视锥体和一个球体。精确的视锥体/球体测试，可以表示为将点 \mathbf{p} 与中间图中的橙色区域和紫色区域进行测试。右边是中间体积的合理近似，如果一个球体的中心位于圆角之外，但是又位于所有外部平面之内，那么这个球体将会被错误地分类为与视锥体相交，即使实际上它位于视锥体外部。
</div>
<br><br>
...
<div style="text-align: center;">
    <img style="border-radius: 0.3125em; box-shadow: 0 2px 4px 0 rgba(34,36,38,.12),0 2px 10px 0 rgba(34,36,38,.08);" src="images/Chapter-22/202310091057533.png">
</div>
<div style="color:orange;display: block;color: #999;padding: 2px; text-align: left;">
    图22.27：图中的粗黑线条代表了视锥体的平面。当使用所介绍的算法对box（左）和视锥体进行相交测试时，如果这个box位于视锥体外部，它可能会被错误地分类为相交。而对于图中所示的情况，当这个box的中心点位于红色区域时，就会发生这种错误分类情况。
</div>
<br><br>
